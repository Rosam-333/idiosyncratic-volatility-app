"""
Idiosyncratic Volatility Analyzer — ORIGINAL scope (Group: Bob Cat).

Pure CAPM-style idiosyncratic volatility vs the market benchmark: metrics, charts,
risk interpretation, and takeaway. No news feed and no buy/hold/sell outlook.

Self-contained for Streamlit Cloud (engine inlined from `bobcat_lab_core.py`).
...
def render_original_dashboard(asset_name, asset_ticker, profile, benchmark_label, frame, metrics):
...
end_download = end_date if end_date <= date.today() else date.today()
if start_date >= end_download:
    st.error("Start date must be earlier than the end date (and before the latest available market data).")
    st.stop()

if not ticker:
    st.error("Enter a valid stock ticker (e.g. AAPL, MSFT).")
    st.stop()

profile = get_company_profile(ticker)
frame = load_asset_vs_benchmark(ticker, STOCK_MARKET_BENCHMARK, start_date, end_download)
if frame is None or frame.empty:
    st.error("No price data was downloaded. Check the ticker symbol and date range.")
    st.stop()
